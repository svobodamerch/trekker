...
class VoiceService:
    ...
    async def _ai_parse(self, transcript: str) -> Dict[str, Any]:
        ...
        prompt = f"""Проанализируй голосовой отчет пользователя о своем дне. Извлеки структурированные данные.
...
Для type="entry" (ежедневный отчет) извлеки:
- mood: число 1-10 или null (настроение)
- energy: число 1-10 или null (энергия)
- anxiety: число 1-10 или null (тревога)
- body_state: строка — описание физических ощущений, что в теле
...
{{
  ...
  "data": {{
    ...
    "energy": 5,
    "anxiety": 3,
    "body_state": "легкое напряжение в плечах",
    "insight": "я слишком критичен к себе",
    "gratitude": "за теплый чай и солнце",
    "today_moment": "во время прогулки в парке",
    "tomorrow_commitment": "сделать 10 минут растяжки",
    "raw_text": "сегодня настроение 7..."
  }}
}}

Если в записи упоминаются цели, планы на будущее ("хочу", "мечтаю", "цель") — используй type="goal"."""
